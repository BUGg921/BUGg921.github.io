...
const currentAvatar = ref<'primary' | 'secondary'>('primary')
function handleSwitch() {
  clearAllSetTimeout()
  avatarImgRef.value?.classList.add('expanding-avatar-circle')
  setTimeout(() => {
    currentAvatar.value = currentAvatar.value === 'primary' ? 'secondary' : 'primary'
...
<template>
  <ShadowCard class="justify-between !p-[5px]">
    ...
    <div class="pointer-events-none relative min-h-[calc(100%_-_50px)] p-6">
      <div class="relative mb-[30px] h-[96px] w-[96px]">
        <div ref="avatarImgRef" class="avatar-img absolute left-0 top-0 z-20 mb-8 h-[96px] w-[96px] rounded-full dark:bg-[var(--header-avatar-bg)]">
          <Icon v-if="currentAvatar === 'primary'" icon="fluent-emoji:technologist" class="h-full w-full" />
          <Icon v-else icon="fluent-emoji:rocket" class="h-full w-full" />
        </div>
      ...
      <button class="switch-btn pointer-events-auto" @click="handleSwitch">
        <div ref="iconRef" class="icon" :class="currentAvatar === 'primary' ? 'icon_rotate_one' : 'icon_rotate_two'" style="transform: rotate(0);">
          <svg
            width="15.702"
            height="15.702"
    ...
    <div class="flex flex-row justify-end gap-4 px-6">
      ...
      <a title="GitHub" class="detail-arrow" :href="SITE.githubHome" target="_blank" rel="noreferrer">
